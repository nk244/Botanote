...
void main() {
  group('ExportService.parseImagePathList', () {
    ...
    test("'|' を含まない単一パスも1件として返す", () {
      expect(
        ExportService.parseImagePathList('images/notes/a_0.jpg'),
    ...
    test('DB 由来の JSON 配列を分解できる', () {
      expect(
        ExportService.parseImagePathList('["/data/app/a.jpg","/data/app/b.jpg"]'),
    ...
    test('リストがそのまま渡された場合も文字列リストにする', () {
      expect(
        ExportService.parseImagePathList(['/data/app/a.jpg']),
        ['/data/app/a.jpg'],
      );
    });
    ...
    test("'[' で始まるが壊れた JSON は '|' 区切りとして解釈する", () {
      expect(
        ExportService.parseImagePathList('[broken|images/notes/a_0.jpg'),
        ['[broken', 'images/notes/a_0.jpg'],
      );
    });
  });
}
