...
- **本文の末尾に `Closes #<issue番号>` を必ず入れる**
- **複数の Issue を閉じる場合は、番号ごとにキーワードを繰り返す**
...
> ⚠️ **`Closes #255, #256, #257` とカンマ区切りで書いても、閉じられるのは先頭の #255 だけ。**
> GitHub は番号ごとにキーワードを要求する。実際に PR #265 でこの書き方をして
> #255 しか閉じず、残り8件を手作業でクローズすることになった。
>
> ⚠️ コミットメッセージの `(#239)` は GitHub の自動クローズ構文ではないため、
> これだけではマージしても Issue が open のまま残る。実際に PR #253 で13件のIssueが
> 閉じ忘れとなった。**Issue を閉じる責任は PR 本文の `Closes #N` が負う**と決めておく。
